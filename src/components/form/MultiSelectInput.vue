...
const props = withDefaults(
  defineProps<{
    label: string
    options: { label: string; value: T; disabled?: boolean; placeholder?: boolean }[]
    multiple?: boolean
    showLabel?: boolean
    placeholderValue?: T
  }>(),
...
const model = defineModel<typeof props.multiple extends boolean ? T[] : T>()

const slugLabel = computed(() => slugify(props.label))
const buttonLabel = computed(() => {
  if (props.multiple && Array.isArray(model.value))
    return model.value.map((item) => item).join(', ')
  return props.options.find((option) => option.value === model.value)?.label
})

const hasPlaceholder = computed(() => {
  if (props.placeholderValue === undefined) return false
  if (Array.isArray(model.value)) return model.value[0] === props.placeholderValue
  return model.value === props.placeholderValue
})
</script>
  ...
  <label :for="`${slugLabel}-dropdown`">
    ...
    <Listbox v-model="model" :multiple="multiple">
      <div class="relative mt-1">
        ...
        >
          <span class="block truncate" :class="{ 'text-gray-400': hasPlaceholder }">
            {{ buttonLabel }}
          </span>
